...
class ResNet(nn.Module):
    ...
    def _forward(self, images, composite_images=None, targets=None, gt_quadrants=None,
                 xe_criterion=None, criterion=None, eval=False, vanilla=False, return_feats=False):
        """
        ...
        :param gt_quadrants:
        :param xe_criterion:
        :param criterion: Could be L1, L2 or any distance metric
        ...
        # Replacing global average pooling with global max pooling
        x = self.maxpool_last(images_feats)
        x = torch.flatten(x, 1)
        images_outputs = self.fc(x)

        y = self.conv1(composite_images)
        y = self.bn1(y)
        y = self.relu(y)
        y = self.maxpool(y)
        y = self.layer1(y)
        ...
        # Replacing global average pooling with global max pooling
        y = self.maxpool_last(composite_images_feats)
        y = torch.flatten(y, 1)
        ...
        gcam_loss = criterion(composite_gradcam_mask, gradcam_gt_mask)
