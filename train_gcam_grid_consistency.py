...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, l1_criterion, optimizer, epoch, args)
...
def train(train_loader, model, criterion, l1_criterion, optimizer, epoch, args):
    ...
    for i, (images, targets, composite_images, gt_quadrants) in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)
        
        images = images.cuda(args.gpu, non_blocking=True)
        targets = targets.cuda(args.gpu, non_blocking=True)
        composite_images = composite_images.cuda(args.gpu, non_blocking=True)
        gt_quadrants = gt_quadrants.cuda(args.gpu, non_blocking=True)

        output, targets, xe_loss, gcam_loss = model(images, composite_images, targets, gt_quadrants, criterion, l1_criterion)
        ...
        loss = xe_loss + args.lambda_val * gcam_loss
